...
with open(open_cdt_file, "r") as cdt_file: 	#need to open cdt file
	for line in cdt_file:
		...
		gene_id_ORF_dict.update({gene_id: ORF})	#need two dictionaries, one for converting the user input ORF to a gene id for the first function, and one to convert gene ids back to ORFs in the second function	
...
off from the highest node. However, it will only record the gene ids (leaves) found at the bottom of the tree, not the nodes. Because you find the gene ids, the function then goes back into your ORF/gene id dictionary to only print out the ORFS, which are what you want to use for GO Term analysis.
"""
